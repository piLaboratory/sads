...
\title{Fitting species abundance model with maximum likelihood \\ Quick reference for \code{sads} package}
...
from communitie, withg and without aggregation of individuals of the
same species.


\section{Installation}

Before installing \code{sads} package please install the dependencies, which are the \R \  
packages:


\begin{itemize}
\item \code{bbmle} \citep{Bolkerbbmle}
\item \code{poilog} \citep{grotan2008}
\item \code{VGAM} \citep{yee2012}
\item \code{untb} \citep{Hankin2007}
\end{itemize}

And then download the beta version of \code{sads} from 
\url{http://ecologia.ib.usp.br/let/doku.php?id=engl:tutorials:rcode#species_abundance_distributions}.
and the install the package. For Linux and MAC OS:
@ 
<<installation-linux, eval=FALSE>>=
install.packages("[dir]/sads_0.1.06.tar.gz", repos=NULL)
@


and for windows:

<<installation-windows, eval=FALSE>>=
install.packages("[dir]/sads_0.1.06.zip", repos=NULL)
@ %def 

where \texttt{[dir]} is the directory where you saved the installation file.
...
The octave number is the lower limite of the class in log2 scale. 
...
To get likelihood profile and confidence intervals use:
...
and \code{plot} to plot at  chi-square scale (square-root of twice the relative log-likelihood):
...
\begin{shaded}
  ...
  from rank-abundance relationships from the correspodending sads and rads models \citep{wilson2009}. 
...
and then you cab use the function \code{AICtab} and friends of the \emph{bbmle} package to get a model selection table:
...
of community with 10 species that follows a lognormal distribution
...
And a density plot with lines indicating the mean of estimates and the values of parameters:
@ 
