...
The package is available on  CRAN, and thus can be installed in a \R session with the command:

@ 
<<installation-and <- load, eval=FALSE>>=
...
Through this document we use two data sets of abundances from the sads
package. To information about these data please refer to their help
pages: 
...
A logical argument \code{preston} allows to smooth the numbers as proposed by \citet{Preston1948}. 
...
and the second argument is the name of the model to be fitted.
...
Rank-abundance models assigns a probability for each
...
The package project is hosted on GitHub
(\url{https://github.com/piklprado/sads/}. Please report bugs and
issues and give us your feedback at
