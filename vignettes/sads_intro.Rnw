...
legend("topright", c("Preston's birds", "Fisher's moths"),
...
\begin{shaded}
  \textbf{Likelihood intervals and confidence intervals:} \hfill
...
\begin{shaded}
  ...
  fits by \code{fitsad}, \code{fitrad} of \emph{sads} and fits by \code{radfit} of \emph{vegan} 
...
often occurs through categorization. For instance, when assessing
...
The function \code{fitsadC} fits continuous distributions commonly
used to describe this type of data. The code below fits all available
models for abundance-class data in the \code{sads} package. Note that
the data must be passed as a \code{histogram} object. Additionally, we
set the \code{trunc.max} argument to the maximum possible cover value
for this dataset (100\%) to right-truncate the distribution.

@
<<grasslands model fit >>=
grass.e <- fitsadC(grass.h, 'exp', trunc.max = 100) # Exponential
grass.g <- fitsadC(grass.h, 'gamma', trunc.max = 100) # Pareto
grass.l <- fitsadC(grass.h, 'lnorm', trunc.max = 100) # Log-normal
grass.p <- fitsadC(grass.h, 'pareto', trunc.max = 100) # Pareto
grass.w <- fitsadC(grass.h, 'weibull', trunc.max = 100) # Weibull
@ %def
