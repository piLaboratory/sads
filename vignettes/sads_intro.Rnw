...
to empirical SADs. These functions can also evaluate fits and compare competing
models. The package also allows the simulation of SADs expected from communities' samples, with and without aggregation of individuals of the same species.
...
The current developer version may be installed from GitHub with:
...
\footnote{\code{fitrad},which also accepts a rank-abundance table returned by function \code{rad} as its first argument.} 
...
\citep{Bolkerbbmle}, and from all other usual methods for model objects, such as
...
\begin{shaded}
  ...
  probability to data in different ways. Function \code{fitsad} fits PDFs to observed abundances and \code{fitrad} fits PDFs 
...
The package project is hosted on GitHub (\url{https://github.com/piklprado/sads/}). 
Please report bugs and issues and give us your feedback at 
\url{https://github.com/piklprado/sads/issues}.
