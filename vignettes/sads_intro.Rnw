...
\begin{shaded}
  ...
  This difference is the log-likehood ratio and is set with the argument \code{ratio} 
  ...
  is highly asymetric or have local minima. But in this kind of profile usually indicates an ill-behaved
  fit, and so the intervals will not meaningfull anyway.
\end{shaded}
