...
The package is available on  CRAN. To install and load the package \code{sads} open an \R session and type:
...
Thorugh this document we use two data sets of abundances from the sads package. To information about these data please refer to their help pages:
