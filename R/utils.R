...
qfinder <- function(dist, want, coef, pad=FALSE) {
	if (any(sapply(coef, length) > 1)) stop("Vectorization not implemented for the parameters")
	dist <- get(paste("d",dist, sep=""), mode="function")
	f <- function(to, from) {
		z <- cumsum(do.call(dist, c(list(x=from:to), coef)))
		y <- z[length(z)]
		return (y)
	}
	...
	upper <- guess
	repeat {
		print(paste("guess:",guess, "val:", my.q))
		if (upper-lower == 1) break;
		guess <- round((lower + upper)/2)
		my.q <- do.call(f, guess)
		if(my.q > want) upper <- guess else lower <- guess
	}
	# Phase 3: lower or upper?
	if (do.call(f, lower) < want) upper else lower
}

qfinder.orig <- function(dist, want, coef) {
	if (any(sapply(coef, length) > 1)) stop("Vectorization not implemented for the parameters")
...
# such as gs
cumsumW <- function(f, q, coef, lower.tail, log.p, pad, from=0) {
	if (any(sapply(coef, length) > 1)) stop("Vectorization not implemented for the parameters")
