...
setMethod("radpred", signature(object="missing", sad="character", rad="missing",
          ...
          function(object, sad, rad, coef, trunc, trunc.max, distr=NA, S, N){
              ...
              if (distribution == "discrete"){
                  ## Inverts the survival function via findInterval instead of calling
                  ## [q] directly, to save some computational time (as [q] is
                  ## inneficiently vectorized). findInterval is used instead of approx()
                  ## because inverting a discrete (step-function) CDF is an exact bin
                  ## lookup, not an interpolation problem: approx() requires strictly
                  ## unique x values and warns/collapses whenever the survival function
                  ## has ties. Ties are unavoidable whenever trunc or trunc.max are used
                  ## (the excluded abundance range gets an identical survival value of 1
                  ## or 0), and can also arise from floating-point underflow in the tail
                  ## of large and/or heavy-tailed distributions.
                  y <- 1:N
                  Y <- ppoints(S)
                  ...
                  ## Guards against floating-point overshoot/undershoot: some [p] functions
                  ## built on cumsum() of a density (e.g. pmzsm, pvolkov, pls) can produce
                  ## values fractionally outside [0,1], which would otherwise break the
                  ## non-decreasing requirement of findInterval below.
                  X <- pmin(pmax(X, 0), 1)
                  Xr <- rev(c(1, X))
                  yr <- rev(c(0, y))
                  j <- findInterval(Y, Xr)
                  ab <- rep(NA_real_, length(Y))
                  ab[j >= 1] <- yr[j[j >= 1]]
                  ## Extreme values of abundance are out of bounds for findInterval. Explicit form:
...
setMethod("qqsad",
        ...
        distribution <- distr(sad)
              ...
              if(distribution == "discrete"){
                  ...
                  ## Guards against floating-point overshoot: some [p] functions built on
                  ## cumsum() of a density (e.g. pmzsm, pvolkov, pls) can exceed 1 by a few
                  ## ULPs in the tail, which would otherwise break the non-decreasing
                  ## requirement of findInterval below.
                  p <- pmin(pmax(p, 0), 1)
                  ## Inverts the (already non-decreasing) CDF via findInterval instead of
                  ## approxfun, since approxfun requires strictly unique x values and
                  ## warns/collapses whenever the CDF has ties -- unavoidable whenever
                  ## trunc or trunc.max are used, or from floating-point precision limits.
                  Xr <- c(p, 1)
                  yr <- c(q, 0)
                  Yq <- ppoints(S)
                  j <- findInterval(Yq, Xr)
                  q <- rep(NA_real_, length(Yq))
                  q[j >= 1] <- yr[j[j >= 1]]
              }
        else if(distribution == "continuous"){
            p <- ppoints(S)
