...
setGeneric("trueLL", 
		   def=function(object, dist, coef, trunc, dec.places, breaks, counts, log = TRUE, ...)
...
setMethod("trueLL", signature(object="fitsad", dist="missing", coef="missing", trunc="missing", 
		  ...
		  function(object, dist, coef, trunc, dec.places, breaks, counts, log = TRUE, ...){
...
setMethod("trueLL", signature(object="numeric", dist="character", coef="list", trunc="ANY", 
		  ...
		  function(object, dist, coef, trunc, dec.places, breaks, counts, log = TRUE, ...){
			  dots <- list(...)
			  if(! missing(breaks) & ! missing(dec.places)) 
				  stop("Unable to provide both dec.places and breaks")
			  if(missing(breaks)){
				  if(missing(dec.places)) dec.places=0
				  D <- 10^(-dec.places)/2
